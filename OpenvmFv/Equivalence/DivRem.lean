...
namespace Equivalence.DivRem
  ...
  section RISC_V_equivalence
    ...
    := by
      ...
      obtain h_op | h_op | h_op | h_op := h_opcode
      . have h_op' := Transpiler.transpiler_opcode_596 h_transpile.1 (by simp [h_transpile]; assumption)
        obtain ⟨ rs2, rs1, rd, h_instr, h_nrd ⟩ := h_op'
      ...
      . have h_op' := Transpiler.transpiler_opcode_597 h_transpile.1 (by simp [h_transpile]; assumption)
        obtain ⟨ rs2, rs1, rd, h_instr, h_nrd ⟩ := h_op'
        subst instruction; simp [Transpiler.transpile_op] at h_transpile
        have h_nrd' : ¬ ((rd == regidx.Regidx 0#5) = true) := by obtain ⟨ rd, prd ⟩ := rd; simp at h_nrd prd; interval_cases rd <;> simp at h_nrd ⊢ <;> rfl
        rw [if_neg h_nrd'] at h_transpile
        ...
        simp [Transpiler.wrap_to_regidx, get_instruction_fields_row, ← h_transpile.2.2.2.2.1, Transpiler.ind, regidx_to_fin]
        obtain ⟨ rd, prd ⟩ := rd; simp at h_nrd prd ⊢; clear *- h_nrd prd; simp [← BitVec.toNat_inj] at h_nrd; omega
      . have h_op' := Transpiler.transpiler_opcode_598 h_transpile.1 (by simp [h_transpile]; assumption)
        obtain ⟨ rs2, rs1, rd, h_instr, h_nrd ⟩ := h_op'
        subst instruction; simp [Transpiler.transpile_op] at h_transpile
        have h_nrd' : ¬ ((rd == regidx.Regidx 0#5) = true) := by obtain ⟨ rd, prd ⟩ := rd; simp at h_nrd prd; interval_cases rd <;> simp at h_nrd ⊢ <;> rfl
        rw [if_neg h_nrd'] at h_transpile
        ...
        simp [Transpiler.wrap_to_regidx, get_instruction_fields_row, ← h_transpile.2.2.2.2.1, Transpiler.ind, regidx_to_fin]
        obtain ⟨ rd, prd ⟩ := rd; simp at h_nrd prd ⊢; clear *- h_nrd prd; simp [← BitVec.toNat_inj] at h_nrd; omega
      . have h_op' := Transpiler.transpiler_opcode_599 h_transpile.1 (by simp [h_transpile]; assumption)
        obtain ⟨ rs2, rs1, rd, h_instr, h_nrd ⟩ := h_op'
        subst instruction; simp [Transpiler.transpile_op] at h_transpile
        have h_nrd' : ¬ ((rd == regidx.Regidx 0#5) = true) := by obtain ⟨ rd, prd ⟩ := rd; simp at h_nrd prd; interval_cases rd <;> simp at h_nrd ⊢ <;> rfl
        rw [if_neg h_nrd'] at h_transpile
    ...
    := by
      ...
      . simp [write_xreg, Sail.writeReg, PreSail.writeReg, cast, and_assoc]
        . by_cases h_rs2 : Transpiler.wrap_to_regidx (air.adapter.rs2_ptr row 0) = 0 <;>
    ...
    :
      (bus_effect (_executionBus_row air row) (_memoryBus_row air row) state).2 =
        let val := U32.toBV #v[air.core.a row 0 0, air.core.a row 0 1, air.core.a row 0 2, air.core.a row 0 3]
    ...
    := by
      ...
      . by_cases h_rs2 : Transpiler.wrap_to_regidx (air.adapter.rs2_ptr row 0) = 0 <;> simp [h_rs2, and_assoc] at h_bus ⊢
