...
section RISC_V_equivalence
    ...
    := by
      ...
      have : (air.core.ctx row 0).instruction.opcode = 592 := by simp [← Valid_MultiplicationCoreAir_4_8.ctx.instruction.opcode_def]

      have h_op' := Transpiler.transpiler_opcode_592 h_transpile.1 (by simp [h_transpile]; assumption)
      obtain ⟨ rs2, rs1, rd, srs1, srs2, h_instr, h_nrd ⟩ := h_op'
