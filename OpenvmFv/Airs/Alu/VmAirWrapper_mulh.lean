...
#define_air "VmAirWrapper_mulh" using "openvm_encapsulation" where
  MainSubAir["adapter": "Rv32MultAdapterAir" width := 18]
  MainSubAir["core": "MulHCoreAir_4_8" width := 21]

-- def Valid_VmAirWrapper_mul.rs2_limbs
--   [Field F]
--   [Field ExtF]
--   (c : Valid_VmAirWrapper_mul F ExtF)
--   (row rotation : ℕ)
-- : Fin 4 → F :=
--   (c.core.ctx row rotation).reads 1

-- def Valid_VmAirWrapper_mul.rs2_sign
--   [Field F]
--   [Field ExtF]
--   (c : Valid_VmAirWrapper_mul F ExtF)
--   (row rotation : ℕ)
...
-- def Valid_VmAirWrapper_mul.rs2_imm
--   [Field F]
--   [Field ExtF]
--   (c : Valid_VmAirWrapper_mul F ExtF)
--   (row rotation : ℕ)
...
-- lemma VmAirWrapper_mul.rs2_imm_def
--   [Field F]
--   [Field ExtF]
--   (c : Valid_VmAirWrapper_mul F ExtF)
--   (row rotation: ℕ)
...
-- @[openvm_encapsulation]
-- lemma VmAirWrapper_mul.rs2_sign_limbs
--   [Field F]
--   [Field ExtF]
--   (c : Valid_VmAirWrapper_mul F ExtF)
--   (row rotation: ℕ)
...
-- @[openvm_encapsulation]
-- lemma VmAirWrapper_mul.rs2_sign_check
--   [Field F]
--   [Field ExtF]
--   (c : Valid_VmAirWrapper_mul F ExtF)
--   (row rotation: ℕ)
