import LeanZKCircuit.OpenVM.Circuit
import LeanZKCircuit.Command.Air.define_air

import OpenvmFv.Airs.LoadStore.LoadStoreCoreAir
import OpenvmFv.Airs.LoadStore.Rv32LoadStoreAdapterAir

set_option linter.unusedVariables false

#define_air "VmAirWrapper_loadstore" using "openvm_encapsulation" where
  MainSubAir["adapter": "Rv32LoadStoreAdapterAir" width := 23]
  MainSubAir["core": "LoadStoreCoreAir_4" width := 18]

def Valid_VmAirWrapper_loadstore.is_store
  [Field F] [Field ExtF]
  (c : Valid_VmAirWrapper_loadstore F ExtF)
  (row rotation : ℕ)
: F :=
  c.core.is_valid row rotation -
  c.core.is_load row rotation

@[openvm_encapsulation]
lemma VmAirWrapper_loadstore.is_store_def
  [Field F] [Field ExtF]
  (c : Valid_VmAirWrapper_loadstore F ExtF)
  (row rotation : ℕ)
:
  c.core.is_valid row rotation -
  c.core.is_load row rotation =
  c.is_store row rotation
:= rfl

def Valid_VmAirWrapper_loadstore.shift_amount
  [Field F] [Field ExtF]
  (c : Valid_VmAirWrapper_loadstore F ExtF)
  (row rotation : ℕ)
: F :=
  c.core.load_shift_amount row rotation +
  c.core.store_shift_amount row rotation

@[openvm_encapsulation]
lemma VmAirWrapper_loadstore.shift_amount_def
  [Field F] [Field ExtF]
  (c : Valid_VmAirWrapper_loadstore F ExtF)
  (row rotation : ℕ)
:
  c.core.load_shift_amount row rotation +
  c.core.store_shift_amount row rotation =
  c.shift_amount row rotation
:= rfl

def Valid_VmAirWrapper_loadstore.read_as
  [Field F] [Field ExtF]
  (c : Valid_VmAirWrapper_loadstore F ExtF)
  (row rotation : ℕ)
: F :=
  c.core.is_load row rotation * c.adapter.mem_as row rotation +
  (1 - c.core.is_load row rotation)

@[openvm_encapsulation]
lemma VmAirWrapper_loadstore.read_as
  [Field F] [Field ExtF]
  (c : Valid_VmAirWrapper_loadstore F ExtF)
  (row rotation : ℕ)
:
  c.core.is_load row rotation * c.adapter.mem_as row rotation +
  (1 - c.core.is_load row rotation) =
  c.read_as row rotation
:= rfl

def Valid_VmAirWrapper_loadstore.read_ptr
  [Field F] [Field ExtF]
  (c : Valid_VmAirWrapper_loadstore F ExtF)
  (row rotation : ℕ)
: F :=
  c.core.is_load row rotation * c.adapter.mem_ptr row rotation +
  (1 - c.core.is_load row rotation) * c.adapter.rd_rs2_ptr row rotation -
  c.core.load_shift_amount row rotation

@[openvm_encapsulation]
lemma VmAirWrapper_loadstore.read_ptr_def
  [Field F] [Field ExtF]
  (c : Valid_VmAirWrapper_loadstore F ExtF)
  (row rotation : ℕ)
:
  c.core.is_load row rotation * c.adapter.mem_ptr row rotation +
  (1 - c.core.is_load row rotation) * c.adapter.rd_rs2_ptr row rotation -
  c.core.load_shift_amount row rotation =
  c.read_ptr row rotation
:= rfl

def Valid_VmAirWrapper_loadstore.write_as
  [Field F] [Field ExtF]
  (c : Valid_VmAirWrapper_loadstore F ExtF)
  (row rotation : ℕ)
: F :=
  c.core.is_load row rotation +
  (1 - c.core.is_load row rotation) * c.adapter.mem_as row rotation

@[openvm_encapsulation]
lemma VmAirWrapper_loadstore.write_as_def
  [Field F] [Field ExtF]
  (c : Valid_VmAirWrapper_loadstore F ExtF)
  (row rotation : ℕ)
:
  c.core.is_load row rotation +
  (1 - c.core.is_load row rotation) * c.adapter.mem_as row rotation =
  c.write_as row rotation
:= rfl

def Valid_VmAirWrapper_loadstore.write_ptr
  [Field F] [Field ExtF]
  (c : Valid_VmAirWrapper_loadstore F ExtF)
  (row rotation : ℕ)
: F :=
  c.core.is_load row rotation * c.adapter.rd_rs2_ptr row rotation +
  (1 - c.core.is_load row rotation) * c.adapter.mem_ptr row rotation -
  c.core.store_shift_amount row rotation

@[openvm_encapsulation]
lemma VmAirWrapper_loadstore.write_ptr_def
  [Field F] [Field ExtF]
  (c : Valid_VmAirWrapper_loadstore F ExtF)
  (row rotation : ℕ)
:
  c.core.is_load row rotation * c.adapter.rd_rs2_ptr row rotation +
  (1 - c.core.is_load row rotation) * c.adapter.mem_ptr row rotation -
  c.core.store_shift_amount row rotation =
  c.write_ptr row rotation
:= rfl

def Valid_VmAirWrapper_loadstore.to_pc
  [Field F] [Field ExtF]
  (c : Valid_VmAirWrapper_loadstore F ExtF)
  (row rotation : ℕ)
: F :=
  c.adapter.from_state.pc row rotation + 4

@[openvm_encapsulation]
lemma VmAirWrapper_loadstore.to_pc_def
  [Field F] [Field ExtF]
  (c : Valid_VmAirWrapper_loadstore F ExtF)
  (row rotation : ℕ)
:
  c.adapter.from_state.pc row rotation + 4 =
  c.to_pc row rotation
:= rfl
