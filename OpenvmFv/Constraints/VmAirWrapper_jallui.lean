...
namespace VmAirWrapper_jallui.constraints
  ...
  section constraint_simplification
    ...
    section interactions
      ...
      def executionBus_row (air : Valid_VmAirWrapper_jallui F ExtF) (row : ℕ) : List (F × List F) :=
        ...
        (air.core.is_valid row 0, [air.to_pc row 0, air.adapter.inner.from_state.timestamp row 0 + 1])]
      ...
      def memoryBus_row (air : Valid_VmAirWrapper_jallui F ExtF) (row : ℕ) : List (F × List F) :=
        [(2013265920 * air.adapter.needs_write row 0,
          [1, air.adapter.inner.rd_ptr row 0, air.adapter.inner.rd_aux_cols.prev_data_0 row 0,
            air.adapter.inner.rd_aux_cols.prev_data_1 row 0, air.adapter.inner.rd_aux_cols.prev_data_2 row 0,
            air.adapter.inner.rd_aux_cols.prev_data_3 row 0, air.adapter.inner.rd_aux_cols.base.prev_timestamp row 0]),
        (air.adapter.needs_write row 0,
          [1, air.adapter.inner.rd_ptr row 0, air.core.rd_data_0 row 0, air.core.rd_data_1 row 0,
            air.core.rd_data_2 row 0, air.core.rd_data_3 row 0, air.adapter.inner.from_state.timestamp row 0])]
      ...
      def rangeCheckerBus_row (air : Valid_VmAirWrapper_jallui F ExtF) (row : ℕ) : List (F × List F) :=
        ...
        (air.adapter.needs_write row 0,
          [air.adapter.inner.rd_aux_cols.base.timestamp_lt_aux.lower_decomp_1 row 0, 12])]
      ...
      def programBus_row (air : Valid_VmAirWrapper_jallui F ExtF) (row : ℕ) : List (F × List F) :=
        [(air.core.is_valid row 0,
          [air.adapter.inner.from_state.pc row 0, air.core.expected_opcode row 0, air.adapter.inner.rd_ptr row 0, 0,
            air.core.imm row 0, 1, 0, air.adapter.needs_write row 0, 0])]
      ...
      def bitwiseBus_row (air : Valid_VmAirWrapper_jallui F ExtF) (row : ℕ) : List (F × List F) :=
        ...
        (air.core.is_jal row 0, [air.core.rd_data_3 row 0, 192, air.core.rd_data_3 row 0 + 192, 1])]
      ...
      def constrain_interactions (air : Valid_VmAirWrapper_jallui F ExtF) : Prop :=
      air.buses = fun index ↦
  ...
  section bus_entries
    ...
    @[simp]
    def busRow [Field ExtF] (air : Valid_VmAirWrapper_jallui FBB ExtF) (row : ℕ)
    : List (FBB × List FBB) :=
    ...
    @[simp]
    def wf_propertiesToAssumePerRow [Field ExtF] (air : Valid_VmAirWrapper_jallui FBB ExtF) (row : ℕ)
    : Prop :=
    ...
    @[simp]
    def wf_propertiesToAssertPerRow [Field ExtF] (air : Valid_VmAirWrapper_jallui FBB ExtF) (row : ℕ)
    : Prop :=
...
end VmAirWrapper_jallui.constraints
