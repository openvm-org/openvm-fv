...
namespace PureSpec
  ...
  def execute_LOADBU_lbu_pure (input : LbuInput) : LbuOutput := {
  ...
  set_option maxHeartbeats 0 in
  lemma execute_LOADBU_lbu_pure_equiv
    (lbu_input : LbuInput)
    (h_assumptions : lbu_state_assumptions lbu_input state)
  :
    (
      do
    ...
    let lbu_output := execute_LOADBU_lbu_pure lbu_input
    (do
      Sail.writeReg Register.nextPC lbu_output.nextPC
  ...
  := by
    ...
    replace h_r1_val := r1_of_write_state lbu_input.PC h_r1_val
    replace h_mstatus := mstatus_of_write_state lbu_input.PC h_mstatus
    replace h_cur_privilege := cur_privilege_of_write_state lbu_input.PC h_cur_privilege
    replace h_clint_base := clint_base_of_write_state lbu_input.PC h_clint_base
    replace h_clint_size := clint_size_of_write_state lbu_input.PC h_clint_size
    replace h_plat_ram_base := ram_base_of_write_state lbu_input.PC h_plat_ram_base
    replace h_plat_ram_size := ram_size_of_write_state lbu_input.PC h_plat_ram_size
    replace h_plat_rom_base := rom_base_of_write_state lbu_input.PC h_plat_rom_base
    replace h_plat_rom_size := rom_size_of_write_state lbu_input.PC h_plat_rom_size
    replace h_htif_tohost_base := htif_tohost_base_of_write_state lbu_input.PC h_htif_tohost_base
    ...
    . simp [Sail.BitVec.addInt]
      rewrite [
        wX_write_xreg_equiv _ _
          (regidx.Regidx lbu_input.rd)
          ⟨lbu_input.rd.toNat, range lbu_input.rd h_rd⟩
      ]
      . split <;> aesop
      . simp
