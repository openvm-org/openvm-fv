...
namespace PureSpec
  ...
  :
    ...
    let beq_output := execute_BGEU_pure beq_input
    (do
  ...
  := by
    ...
    simp
    by_cases h_eq: beq_input.r2_val.toNat ≤  beq_input.r1_val.toNat
    . simp [h_eq]
      ...
      by_cases h_throws : (execute_BGEU_pure beq_input).throws
      . simp [execute_BGEU_pure, h_eq, h_input_imm] at h_throws
        simp [h_throws, execute_BGEU_pure, h_eq, h_input_imm]
      . simp [execute_BGEU_pure, h_eq, h_input_imm] at h_throws
        simp [h_throws, execute_BGEU_pure, h_eq, h_input_imm]
