...
def target_bit_0_invalid (input : BeqInput) : Bool :=
  BitVec.ofBool (input.PC + BitVec.signExtend 32 input.imm)[0]! == 1#1

def target_bit_1_invalid (input: BeqInput): Bool :=
...
:
  (
    do
      ...
      LeanRV32D.Functions.execute (instruction.BTYPE (imm, r1, r2, bop.BEQ ))
  ) state =
...
:= by
  ...
  simp
  by_cases h_eq: beq_input.r2_val = beq_input.r1_val
  . simp [h_eq]
