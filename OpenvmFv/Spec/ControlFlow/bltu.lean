...
namespace PureSpec
  ...
  :
    (
      do
        ...
        LeanRV32D.Functions.execute (instruction.BTYPE (imm, r2, r1, bop.BLTU ))
    ) state =
  ...
  := by
    ...
    simp
    by_cases h_eq: beq_input.r1_val.toNat < beq_input.r2_val.toNat
    . simp [h_eq]
      ...
      by_cases h_throws : (execute_BLTU_pure beq_input).throws
      . simp [execute_BLTU_pure, h_eq, h_input_imm] at h_throws
        simp [h_throws, execute_BLTU_pure, h_eq, h_input_imm]
      . simp [execute_BLTU_pure, h_eq, h_input_imm] at h_throws
        simp [h_throws, execute_BLTU_pure, h_eq, h_input_imm]
        by_cases h_success : (execute_BLTU_pure beq_input).success
        . simp [execute_BLTU_pure, h_eq, h_throws, h_input_imm] at h_success
