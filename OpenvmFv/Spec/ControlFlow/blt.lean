...
namespace PureSpec
  ...
  :
    (
      do
        ...
        LeanRV32D.Functions.execute (instruction.BTYPE (imm, r2, r1, bop.BLT ))
    ) state =
    let beq_output := execute_BLT_pure beq_input
    (do
  ...
  := by
    ...
    simp
    by_cases h_eq: beq_input.r1_val.toInt < beq_input.r2_val.toInt
    . simp [h_eq]
      ...
      by_cases h_throws : (execute_BLT_pure beq_input).throws
      . simp [execute_BLT_pure, h_eq, h_input_imm] at h_throws
        simp [h_throws, execute_BLT_pure, h_eq, h_input_imm]
      . simp [execute_BLT_pure, h_eq, h_input_imm] at h_throws
        simp [h_throws, execute_BLT_pure, h_eq, h_input_imm]
        by_cases h_success : (execute_BLT_pure beq_input).success
        . simp [execute_BLT_pure, h_eq, h_throws, h_input_imm] at h_success
