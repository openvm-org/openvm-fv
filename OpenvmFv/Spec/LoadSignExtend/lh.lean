...
namespace PureSpec
  ...
  def execute_LOADH_lh_pure (input : LhInput) : LhOutput := {
    ...
    rd := if h: input.rd = 0
      then .none
        ...
        ⟨
          ...
          range input.rd h
        ⟩,
        (BitVec.extend (input.data1 ++ input.data0) 32 true)
      )
    : LhOutput
  }
  ...
  set_option maxHeartbeats 0 in
  lemma execute_LOADH_lh_pure_equiv
    (lh_input : LhInput)
    (h_assumptions : lh_state_assumptions lh_input state)
  :
    (
      do
    ...
    let lh_output := execute_LOADH_lh_pure lh_input
    (do
      Sail.writeReg Register.nextPC lh_output.nextPC
  ...
  := by
    ...
    replace h_r1_val := r1_of_write_state lh_input.PC h_r1_val
    replace h_mstatus := mstatus_of_write_state lh_input.PC h_mstatus
    replace h_cur_privilege := cur_privilege_of_write_state lh_input.PC h_cur_privilege
    replace h_clint_base := clint_base_of_write_state lh_input.PC h_plat_clint_base_val
    replace h_clint_size := clint_size_of_write_state lh_input.PC h_plat_clint_size_val
    replace h_plat_ram_base := ram_base_of_write_state lh_input.PC h_plat_ram_base_val
    replace h_plat_ram_size := ram_size_of_write_state lh_input.PC h_plat_ram_size
    replace h_plat_rom_base := rom_base_of_write_state lh_input.PC h_plat_rom_base_val
    replace h_plat_rom_size := rom_size_of_write_state lh_input.PC h_plat_rom_size_val
    replace h_htif_tohost_base := htif_tohost_base_of_write_state lh_input.PC h_plat_htif_tohost_base_val

    have h_execute_load := Local.execute_LOADH_simplified
      (write_reg_state state Register.nextPC (Sail.BitVec.addInt lh_input.PC 4))
      (regidx.Regidx lh_input.rd)
      lh_input.data0
      lh_input.data1
      h_aligned
    ...
    . simp [Sail.BitVec.addInt]
      rewrite [
        wX_write_xreg_equiv _ _
          (regidx.Regidx lh_input.rd)
          ⟨lh_input.rd.toNat, range lh_input.rd h_rd⟩
      ]
      . split <;> aesop
      . simp
