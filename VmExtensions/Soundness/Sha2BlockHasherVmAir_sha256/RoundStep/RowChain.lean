...
One round row processes 4 consecutive SHA-256 rounds (slots 0–3).
...
private theorem round_step_constraints_of_blockHasherConstraints
    (air : C FBB ExtF) (hc : blockHasherConstraints air) (row : ℕ) :
...
private theorem workvar_bits_of_blockHasherConstraints
  ...
  allWorkVarBitsBoolean_of_constraints air row (hc row).2.1
...
private theorem blockStateRow_le_last
  ...
  · subst hstart
    ...
    | zero =>
        simp [blockStateRow]
    | succ k =>
        have hlast : 16 ≤ Circuit.last_row air := by
          simpa [blockWindowSupported] using hwindow
        simp [blockStateRow]
        omega
  · cases offset with
    | zero =>
        have hlast : start + 16 ≤ Circuit.last_row air := by
          simpa [blockWindowSupported] using hwindow
...
/-- The schedule bits on a full SHA-256 block window are derivable from the
...
theorem slot0_result (air : C FBB ExtF) (start offset : ℕ)
    ...
    let s1 := roundStep prevState (sha256K[baseRound]!) (schedule[baseRound]!)
  ...
  have hrs_local := round_step_constraints_of_blockHasherConstraints air hc srcRow
  have hsched_local := schedule_constraints_of_blockHasherConstraints air hc srcRow
  ...
  have hsched_bits :
    ...
    have hs :=
      schedule_bits_boolean air srcRow hlocal_le hrot hsched_local hround_next
    intro bit
  ...
  · calc
      ...
      _ = (roundStep (slotInputState air srcRow 0)
            (sha256K[offset * 4]!) (scheduleWordAtRow air (nextRow air srcRow) 0)).a := by
            simpa using ha
      _ = (roundStep prevState (sha256K[offset * 4]!)
  ...
  · calc
      eWord air (start + offset) 0 = eWord air (nextRow air srcRow) 0 := by
        simp [hnextlocal]
      _ = (roundStep (slotInputState air srcRow 0)
            (sha256K[offset * 4]!) (scheduleWordAtRow air (nextRow air srcRow) 0)).e := by
            simpa using he
      _ = (roundStep prevState (sha256K[offset * 4]!)
...
theorem slot1_result (air : C FBB ExtF) (start offset : ℕ)
    ...
    let schedule := expandSchedule (blockInputWords air start)
    let s1 := roundStep prevState (sha256K[baseRound]!) (schedule[baseRound]!)
    let s2 := roundStep s1 (sha256K[baseRound + 1]!) (schedule[baseRound + 1]!)
    aWord air row 1 = s2.a ∧ eWord air row 1 = s2.e := by
  dsimp
  let srcRow := blockStateRow air start offset
  let schedule := expandSchedule (blockInputWords air start)
  let s1 := roundStep prevState (sha256K[offset * 4]!) (schedule[offset * 4]!)
  let s2 := roundStep s1 (sha256K[offset * 4 + 1]!) (schedule[offset * 4 + 1]!)
  have hnextlocal : nextRow air srcRow = start + offset := by
    simpa [srcRow] using next_blockStateRow_eq air start offset hwindow hoffset
  have hlocal_le : srcRow ≤ Circuit.last_row air := by
    simpa [srcRow] using blockStateRow_le_last air start offset hwindow hoffset
  ...
  · calc
      ...
      _ = (roundStep (slotInputState air srcRow 1)
            (sha256K[offset * 4 + 1]!) (scheduleWordAtRow air (nextRow air srcRow) 1)).a := by
            simpa using ha
      _ = s2.a := by
            rw [hinput, hsched_trace]
  · calc
      eWord air (start + offset) 1 = eWord air (nextRow air srcRow) 1 := by
        simp [hnextlocal]
      _ = (roundStep (slotInputState air srcRow 1)
            (sha256K[offset * 4 + 1]!) (scheduleWordAtRow air (nextRow air srcRow) 1)).e := by
            simpa using he
      _ = s2.e := by
            rw [hinput, hsched_trace]
...
theorem slot2_result (air : C FBB ExtF) (start offset : ℕ)
    ...
    let s1 := roundStep prevState (sha256K[baseRound]!) (schedule[baseRound]!)
    let s2 := roundStep s1 (sha256K[baseRound + 1]!) (schedule[baseRound + 1]!)
    let s3 := roundStep s2 (sha256K[baseRound + 2]!) (schedule[baseRound + 2]!)
  ...
  let s1 := roundStep prevState (sha256K[offset * 4]!) (schedule[offset * 4]!)
  let s2 := roundStep s1 (sha256K[offset * 4 + 1]!) (schedule[offset * 4 + 1]!)
  let s3 := roundStep s2 (sha256K[offset * 4 + 2]!) (schedule[offset * 4 + 2]!)
  have hnextlocal : nextRow air srcRow = start + offset := by
    simpa [srcRow] using next_blockStateRow_eq air start offset hwindow hoffset
  have hlocal_le : srcRow ≤ Circuit.last_row air := by
    simpa [srcRow] using blockStateRow_le_last air start offset hwindow hoffset
  ...
  · calc
      ...
      _ = (roundStep (slotInputState air srcRow 2)
            (sha256K[offset * 4 + 2]!) (scheduleWordAtRow air (nextRow air srcRow) 2)).a := by
            simpa using ha
      _ = s3.a := by
            rw [hinput, hsched_trace]
  · calc
      eWord air (start + offset) 2 = eWord air (nextRow air srcRow) 2 := by
        simp [hnextlocal]
      _ = (roundStep (slotInputState air srcRow 2)
            (sha256K[offset * 4 + 2]!) (scheduleWordAtRow air (nextRow air srcRow) 2)).e := by
            simpa using he
      _ = s3.e := by
            rw [hinput, hsched_trace]
...
theorem slot3_result (air : C FBB ExtF) (start offset : ℕ)
    ...
    let s2 := roundStep s1 (sha256K[baseRound + 1]!) (schedule[baseRound + 1]!)
    let s3 := roundStep s2 (sha256K[baseRound + 2]!) (schedule[baseRound + 2]!)
    let s4 := roundStep s3 (sha256K[baseRound + 3]!) (schedule[baseRound + 3]!)
  ...
  let s2 := roundStep s1 (sha256K[offset * 4 + 1]!) (schedule[offset * 4 + 1]!)
  let s3 := roundStep s2 (sha256K[offset * 4 + 2]!) (schedule[offset * 4 + 2]!)
  let s4 := roundStep s3 (sha256K[offset * 4 + 3]!) (schedule[offset * 4 + 3]!)
  have hnextlocal : nextRow air srcRow = start + offset := by
    simpa [srcRow] using next_blockStateRow_eq air start offset hwindow hoffset
  have hlocal_le : srcRow ≤ Circuit.last_row air := by
    simpa [srcRow] using blockStateRow_le_last air start offset hwindow hoffset
  ...
  · calc
      ...
      _ = (roundStep (slotInputState air srcRow 3)
            (sha256K[offset * 4 + 3]!) (scheduleWordAtRow air (nextRow air srcRow) 3)).a := by
            simpa using ha
      _ = s4.a := by
            rw [hinput, hsched_trace]
  · calc
      eWord air (start + offset) 3 = eWord air (nextRow air srcRow) 3 := by
        simp [hnextlocal]
      _ = (roundStep (slotInputState air srcRow 3)
            (sha256K[offset * 4 + 3]!) (scheduleWordAtRow air (nextRow air srcRow) 3)).e := by
            simpa using he
      _ = s4.e := by
            rw [hinput, hsched_trace]
...
theorem four_rounds_per_row (air : C FBB ExtF) (start offset : ℕ)
  ...
  dsimp
  have h0 := slot0_result air start offset hoffset hwindow hrot hshape hc hcarry prevState hprev
  have h1 := slot1_result air start offset hoffset hwindow hrot hshape hc hcarry prevState hprev
  have h2 := slot2_result air start offset hoffset hwindow hrot hshape hc hcarry prevState hprev
  have h3 := slot3_result air start offset hoffset hwindow hrot hshape hc hcarry prevState hprev
  rcases h0 with ⟨ha0, he0⟩
  rcases h1 with ⟨ha1, he1⟩
  rcases h2 with ⟨ha2, he2⟩
...
private theorem compressionPrefix_four
    ...
    (compressionPrefix startState schedule (n + 4)).1 =
      ...
      let s1 := roundStep s0 (sha256K[n]!) (schedule[n]!)
      let s2 := roundStep s1 (sha256K[n + 1]!) (schedule[n + 1]!)
      let s3 := roundStep s2 (sha256K[n + 2]!) (schedule[n + 2]!)
      let s4 := roundStep s3 (sha256K[n + 3]!) (schedule[n + 3]!)
      s4 := by
...
theorem compression_trace_step (air : C FBB ExtF) (start : ℕ) (offset : ℕ)
  ...
  have htrace_next :
      (compressionTrace initialState schedule)[baseRound + 4]! =
        ...
        let s1 := roundStep s0 (sha256K[baseRound]!) (schedule[baseRound]!)
        let s2 := roundStep s1 (sha256K[baseRound + 1]!) (schedule[baseRound + 1]!)
        let s3 := roundStep s2 (sha256K[baseRound + 2]!) (schedule[baseRound + 2]!)
        let s4 := roundStep s3 (sha256K[baseRound + 3]!) (schedule[baseRound + 3]!)
        s4 := by
...
theorem compression_trace_per_row (air : C FBB ExtF) (start : ℕ) (offset : ℕ)
  ...
  | succ offset ih =>
      have hoffset' : offset < 16 := by omega
...
theorem compression_trace_final (air : C FBB ExtF) (start : ℕ)
    ...
    (hc : blockHasherConstraints air)
    (hcarry : roundCarryRangeOnBlock air start) :
    carriedState air (blockStateRow air start 16) =
      (compressionTrace (blockStartState air start)
        (expandSchedule (blockInputWords air start)))[64]! := by
  simpa using compression_trace_per_row air start 16 (by omega) hwindow hrot hshape hc hcarry
