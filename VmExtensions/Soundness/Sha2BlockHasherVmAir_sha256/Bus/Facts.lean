/-
Bus-derived proof facts for the SHA-256 block hasher.
-/
...
namespace Sha2BlockHasherVmAir_sha256.Soundness.BusFacts
...
private theorem carry_entry_assume
  ...
  have hidx : 2 * i + j < (_bitwiseBus_row air row).length := by
    have : 2 * i + j < 24 := by omega
    simpa [_bitwiseBus_row] using this
  have hgeneric : Interaction.BusEntry.assume FBB ((_bitwiseBus_row air row)[2 * i + j]) := by
    simpa [bitwise_lookup_send_properties, propertiesToAssume] using
      (forall_id_map_get
        ...
        (n := 2 * i + j)
        hidx)
  have hget : (_bitwiseBus_row air row)[2 * i + j] = bitwiseCarryBusEntry air i j row := by
    have hi_cases : i = 0 ∨ i = 1 ∨ i = 2 ∨ i = 3 := by omega
    have hj_cases : j = 0 ∨ j = 1 := by omega
    rcases hi_cases with rfl | rfl | rfl | rfl <;> rcases hj_cases with rfl | rfl <;>
      rfl
...
private theorem digest_entry_assume
  ...
  have hidx : 8 + (2 * word + pair) < (_bitwiseBus_row air row).length := by
    have : 8 + (2 * word + pair) < 24 := by omega
    simpa [_bitwiseBus_row] using this
  have hgeneric :
      Interaction.BusEntry.assume FBB ((_bitwiseBus_row air row)[8 + (2 * word + pair)]) := by
    simpa [bitwise_lookup_send_properties, propertiesToAssume] using
      (forall_id_map_get
        (f := Interaction.BusEntry.assume FBB)
        (l := _bitwiseBus_row air row)
        ...
        hidx)
  have hget :
      (_bitwiseBus_row air row)[8 + (2 * word + pair)] = bitwiseDigestBusEntry air word pair row := by
    have hw_cases :
        word = 0 ∨ word = 1 ∨ word = 2 ∨ word = 3 ∨
        word = 4 ∨ word = 5 ∨ word = 6 ∨ word = 7 := by
      omega
    have hp_cases : pair = 0 ∨ pair = 1 := by omega
...
theorem digest_final_hash_byte_range_of_bus
  ...
  have hnext_final :
    ...
    · rw [next_final_hash, final_hash, next_main_eq_main_succ air hrot
        (312 + 4 * word + byte) (prevRow air row) hprev_lt]
...
private theorem range_loop_two : List.range.loop 2 [] = [0, 1] := by decide
...
@[simp] theorem privateBus_send_data_get?_hash
    ...
    (hw : word < 8) (hl : limb < 2) :
    (privateBus_send_data air row)[2 * word + limb]? =
      some (composed_hash_u16 air word limb row) := by
  interval_cases word <;> interval_cases limb <;>
    simp [privateBus_send_data, List.range, range_loop_two, range_loop_eight]

@[simp] theorem privateBus_recv_data_get?_prev_hash
    (air : C FBB ExtF) (row word limb : ℕ)
    (hw : word < 8) (hl : limb < 2) :
    (privateBus_recv_data air row)[2 * word + limb]? =
      some (prev_hash air word limb row) := by
  interval_cases word <;> interval_cases limb <;>
    simp [privateBus_recv_data, List.range, range_loop_two, range_loop_eight]
...
end Sha2BlockHasherVmAir_sha256.Soundness.BusFacts
