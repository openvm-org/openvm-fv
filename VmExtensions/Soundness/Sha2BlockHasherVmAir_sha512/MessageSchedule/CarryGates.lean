...
def intermed8CarryGatePoly (d0 d1 d2 d3 d4 d5 : FBB) : FBB :=
...
def intermed12CarryGatePoly (d0 d1 d2 d3 d4 d5 : FBB) : FBB :=
  let s := d0 + d1 + d2 + d3 + d4 + d5
  d4 * (2 - s) +
  d4 * d5 +
  d4 * (d4 - 1) * 1006632961 +
...
theorem intermed8CarryGateNext_eq_one (air : C FBB ExtF) (row : ℕ)
  ...
  have hgate :
    ...
    exact (intermed8_gate_lookup_fin n0 n1 n2 n3 n4 n5 htotal_le hc11).mpr hsel_shift
...
theorem intermed12CarryGateNext_eq_one (air : C FBB ExtF) (row : ℕ)
  ...
  have hgate :
      intermed12CarryGatePoly (n0.val : FBB) (n1.val : FBB) (n2.val : FBB)
        (n3.val : FBB) (n4.val : FBB) (n5.val : FBB) = 1 := by
    exact (intermed12_gate_lookup_fin n0 n1 n2 n3 n4 n5 htotal_le hc11).mpr hsel_shift
