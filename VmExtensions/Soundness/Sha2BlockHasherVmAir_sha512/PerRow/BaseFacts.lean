/-
  ...
  Converts raw flag/selector constraint satisfaction into semantic predicates:
  - `flag_constraints` -> `flagsWellFormed`
  - `flag_constraints` -> first selector/encoder facts

  `SelectorFacts` contains the heavier selector facts.
-/
...
/-- Each of the 6 encoder digits is ternary (constraints 4-9). -/
theorem encoder_digits_ternary (air : C FBB ExtF) (row : ℕ)
  ...
  rcases hf with ⟨_, _, _, _, h4, h5, h6, h7, h8, h9, _, _, _, _, _, _, _⟩
  intro i hi
  interval_cases i
  ...
  · simpa using ternary_of_cube_eq_zero (encoder_idx air 5 row) h9

/-- The sum of the 6 encoder digits is ternary (constraint 10). -/
theorem encoder_digit_sum_ternary (air : C FBB ExtF) (row : ℕ)
  ...
  rcases hf with ⟨_, _, _, _, _, _, _, _, _, _, h10, _, _, _, _, _, _⟩
  simpa [encoder_digit_sum] using ternary_of_cube_eq_zero (encoder_digit_sum air row) h10

/-- Assembly: `flag_constraints` implies `flagsWellFormed`. -/
