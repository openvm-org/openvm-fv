import VmExtensions.Soundness.Sha2MainAir_sha256.PermutationMultiplicity
...
private theorem exists_start_msg1_alignment_of_rawPermutationSemantics
    ...
    ∃ start,
      ...
      Sha2BlockHasherVmAir_sha256.constraints.request_id blockAir start =
      ...
      (Sha2BlockHasherVmAir_sha256.constraints.wrapperMsg1Entry blockAir start).toRaw.2 =
        (Sha2MainAir_sha256.constraints.wrapperMsg1Entry mainAir row).toRaw.2 := by
  let data := (Sha2MainAir_sha256.constraints.wrapperMsg1Entry mainAir row).toRaw.2
  have hmain_mult :
      InteractionList.get_multiplicity
        (Sha2MainAir_sha256.constraints.wrapperBus_trace mainAir) data = 1 := by
    simpa [data] using
      mainMsg1_trace_getMultiplicity_eq_one
        mainAir row hmainc hmain_rot hmain_trace_fit hrow h_enabled
  have hblock_mult :
      InteractionList.get_multiplicity
        (Sha2BlockHasherVmAir_sha256.constraints.wrapperBus_trace blockAir) data = -1 := by
    have hsum :
        InteractionList.get_multiplicity
      ...
      simpa [sharedWrapperTraceEntries, InteractionList.get_multiplicity_append, data] using h_raw_perm data
    have hsum' :
        1 +
          InteractionList.get_multiplicity
            (Sha2BlockHasherVmAir_sha256.constraints.wrapperBus_trace blockAir) data = 0 := by
  ...
  have hexists :
      ∃ start,
        start ≤ Circuit.last_row blockAir ∧
        (Sha2BlockHasherVmAir_sha256.constraints.wrapperMsg1Entry blockAir start).multiplicity = -1 ∧
        (Sha2BlockHasherVmAir_sha256.constraints.wrapperMsg1Entry blockAir start).toRaw.2 = data := by
    by_contra hno
  ...
  have hreq_opt :
      some (Sha2BlockHasherVmAir_sha256.constraints.request_id blockAir start) =
...
private theorem main_state_raw_of_start_request_id_of_rawPermutation
    ...
    (Sha2MainAir_sha256.constraints.wrapperStateEntry mainAir row).toRaw.2 =
      (Sha2BlockHasherVmAir_sha256.constraints.wrapperStateEntry blockAir (start + 16)).toRaw.2 := by
  ...
  have hexists :
      ∃ row',
        ...
        Sha2MainAir_sha256.constraints.is_enabled mainAir row' = 1 ∧
        (Sha2MainAir_sha256.constraints.wrapperStateEntry mainAir row').toRaw.2 = data := by
    by_contra hno
  ...
  have hreq_opt :
      some (Sha2MainAir_sha256.constraints.request_id mainAir row') =
        some (Sha2BlockHasherVmAir_sha256.constraints.request_id blockAir (start + 16)) := by
  ...
  have hreq_row'_16 :
      Sha2MainAir_sha256.constraints.request_id mainAir row' =
        Sha2BlockHasherVmAir_sha256.constraints.request_id blockAir (start + 16) :=
    Option.some.inj hreq_opt
  have hreq_16_start :
      Sha2BlockHasherVmAir_sha256.constraints.request_id blockAir (start + 16) =
        Sha2BlockHasherVmAir_sha256.constraints.request_id blockAir start :=
    block_request_id_eq_start blockAir start 16 (by omega) hstart hsel hrot hc
  have hreq_row'_start :
      Sha2MainAir_sha256.constraints.request_id mainAir row' =
        Sha2BlockHasherVmAir_sha256.constraints.request_id blockAir start :=
    hreq_row'_16.trans hreq_16_start
  have hreq_row'_row :
      Sha2MainAir_sha256.constraints.request_id mainAir row' =
        Sha2MainAir_sha256.constraints.request_id mainAir row :=
  ...
  have hrow_eq : row' = row :=
    mainTraceConstraints.enabled_request_id_injective hmainc
      hmain_rot hmain_trace_fit row' row
      (by omega) hrow h_enabled_row' h_enabled hreq_row'_row
  calc
    (Sha2MainAir_sha256.constraints.wrapperStateEntry mainAir row).toRaw.2 =
        (Sha2MainAir_sha256.constraints.wrapperStateEntry mainAir row').toRaw.2 := by
          simpa [hrow_eq]
    _ = data := hstate_raw'
    _ = (Sha2BlockHasherVmAir_sha256.constraints.wrapperStateEntry blockAir (start + 16)).toRaw.2 := rfl
...
private theorem main_msg2_raw_of_start_request_id_of_rawPermutation
    ...
    (Sha2MainAir_sha256.constraints.wrapperMsg2Entry mainAir row).toRaw.2 =
      (Sha2BlockHasherVmAir_sha256.constraints.wrapperMsg2Entry blockAir (start + 2)).toRaw.2 := by
  ...
  have hexists :
      ∃ row',
        row' ≤ Circuit.last_row mainAir ∧
        Sha2MainAir_sha256.constraints.is_enabled mainAir row' = 1 ∧
        (Sha2MainAir_sha256.constraints.wrapperMsg2Entry mainAir row').toRaw.2 = data := by
    by_contra hno
  ...
  have hreq_opt :
      some (Sha2MainAir_sha256.constraints.request_id mainAir row') =
        some (Sha2BlockHasherVmAir_sha256.constraints.request_id blockAir (start + 2)) := by
  ...
  have hreq_row'_2 :
      Sha2MainAir_sha256.constraints.request_id mainAir row' =
        Sha2BlockHasherVmAir_sha256.constraints.request_id blockAir (start + 2) :=
    Option.some.inj hreq_opt
  have hreq_2_start :
      Sha2BlockHasherVmAir_sha256.constraints.request_id blockAir (start + 2) =
        Sha2BlockHasherVmAir_sha256.constraints.request_id blockAir start :=
  ...
  calc
    (Sha2MainAir_sha256.constraints.wrapperMsg2Entry mainAir row).toRaw.2 =
        (Sha2MainAir_sha256.constraints.wrapperMsg2Entry mainAir row').toRaw.2 := by
          simpa [hrow_eq]
    _ = data := hmsg2_raw'
    _ = (Sha2BlockHasherVmAir_sha256.constraints.wrapperMsg2Entry blockAir (start + 2)).toRaw.2 := rfl
...
theorem exists_start_of_sharedWrapperRawPermutationSemantics
  ...
  have hstate_raw :=
    main_state_raw_of_start_request_id_of_rawPermutation
      mainAir blockAir row start
      hmainc h_enabled hmain_rot hmain_trace_fit hrow
      hrot hc h_raw_perm htrace_fit hstart hsel hreq_start
  have hmsg2_raw :=
