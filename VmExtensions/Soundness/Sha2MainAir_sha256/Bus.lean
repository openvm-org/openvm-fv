...
namespace Sha2MainAir_sha256.Soundness
...
def wrapperPayloadSpec (air : C F ExtF) (row : ℕ) : Prop :=
  ...
  (∀ i : Fin 16, state.prev_state_u16.get i = prev_state_u16 air i.1 row) ∧
  (∀ i : Fin 32, state.new_state_bytes.get i = new_state_byte air i.1 row) ∧
  (∀ i : Fin 16, msg1.local_msg_bytes.get i = message_byte air i.1 row) ∧
  (∀ i : Fin 16, msg1.next_msg_bytes.get i = message_byte air (16 + i.1) row) ∧
  (∀ i : Fin 16, msg2.local_msg_bytes.get i = message_byte air (32 + i.1) row) ∧
  (∀ i : Fin 16, msg2.next_msg_bytes.get i = message_byte air (48 + i.1) row)
...
theorem wrapperPayload_of_row
  ...
  simp [wrapperPayloadSpec, wrapperStateEntry, wrapperMsg1Entry, wrapperMsg2Entry]
...
/-- The theorem-facing raw SHA-256 main-trace package: extracted interactions
plus the extracted per-row constraints. -/
structure mainTraceConstraints (air : C FBB ExtF) : Prop where
  constrain_interactions :
    Sha2MainAir_Sha256Config.extraction.constrain_interactions air
  rows :
    ∀ row, Sha2MainAir_Sha256Config.extraction.constrain_row air row

instance (air : C FBB ExtF) : CoeFun (mainTraceConstraints air) (fun _ => ∀ row,
    Sha2MainAir_Sha256Config.extraction.constrain_row air row) where
  coe hc := hc.rows
...
theorem constraint_0
    ...
    Sha2MainAir_sha256.constraints.constraint_0 air row := by
  exact (Sha2MainAir_sha256.constraints.constraint_0_of_extraction air row).mp (hc row).2.2.1

theorem constraint_1
    {air : C FBB ExtF}
    (hc : mainTraceConstraints air) (row : ℕ) :
    Sha2MainAir_sha256.constraints.constraint_1 air row := by
  exact (Sha2MainAir_sha256.constraints.constraint_1_of_extraction air row).mp (hc row).2.2.2.1

theorem constraint_2
    {air : C FBB ExtF}
    (hc : mainTraceConstraints air) (row : ℕ) :
    Sha2MainAir_sha256.constraints.constraint_2 air row := by
  exact (Sha2MainAir_sha256.constraints.constraint_2_of_extraction air row).mp (hc row).1

theorem constraint_3
    {air : C FBB ExtF}
    (hc : mainTraceConstraints air) (row : ℕ) :
    Sha2MainAir_sha256.constraints.constraint_3 air row := by
  exact (Sha2MainAir_sha256.constraints.constraint_3_of_extraction air row).mp (hc row).2.1
...
theorem request_id_eq_row
  ...
  | 0, _, h_enabled => by
      ...
      have hreq0 : request_id air 0 = 0 := by
        simp [Sha2MainAir_sha256.constraints.constraint_0, Circuit.isFirstRow, h_enabled] at h0
        exact h0
      simpa using hreq0
  | row + 1, hvalid, h_enabled => by
      ...
      have hnext_enabled_eq :
        ...
        simpa [next_is_enabled, is_enabled] using
          next_main_eq_main_succ air hrot 129 row hrow_lt
      ...
      have hprev_enabled_sub :
        ...
        simpa [Sha2MainAir_sha256.constraints.constraint_3, htransition, hnext_enabled] using h3
      ...
      have hnext_request_step :
        ...
        have hnext_request_sub :
          ...
          simpa [Sha2MainAir_sha256.constraints.constraint_1, htransition, hnext_enabled] using h1
...
end Sha2MainAir_sha256.Soundness
