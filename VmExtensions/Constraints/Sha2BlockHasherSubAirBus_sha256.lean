/-
  Bus interaction definitions for Sha2BlockHasherSubAir (SHA-256).
  ...
  The SHA-256 proof boundary derives and uses the supported-row theorem
...
import VmExtensions.Constraints.Sha2WrapperBusEntries_sha256
...
-- These bus indices MUST match the raw circuit extraction
-- (`Sha2BlockHasherVmAir_Sha256Config.extraction.constrain_interactions`), which
-- pins the wrapper-communication payload at index 8 and the private-chaining
-- payload at index 9 (bitwise lookup is at `BitwiseBus = 6`). The proven SHA-512
-- twin uses the identical 8/9 numbering. (A previous revision set these to 28/29
-- to anticipate a composed-VM renumbering; but these lemmas are about the SubAir
-- circuit `c` whose *own* extraction uses 8/9, so 28/29 forced `buses c 28/29 = []`
-- and made the trace bridges false as stated.)
...
lookup for each slot `i` ∈ {0,1,2,3} and limb `j` ∈ {0,1}. (8 entries per row.)

On the **row before a digest row**, the AIR sends
`(next_final_hash[word][2k], next_final_hash[word][2k+1], 0, 0)` for each
word `i` ∈ {0..7} and pair `k` ∈ {0,1}. (16 entries per row.)
...
/-- All bitwise lookup bus entries at a given row.
    Round rows contribute 8 carry entries.
    Pre-digest rows contribute 16 digest byte entries. -/
@[Sha2BlockHasherVmAir_Sha256Config_constraint_and_interaction_simplification]
noncomputable def bitwiseBus_row (c : C F ExtF) (row : ℕ) : List (F × List F) :=
  -- Carry range checks: 4 slots × 2 limbs = 8 entries (gated by is_round_row)
  ((List.range 4).flatMap fun i =>
    (List.range 2).map fun j =>
      bitwiseBus_carry_entry c i j row)
  ++
  -- Digest byte range checks: 8 words × 2 pairs = 16 entries (gated by next_is_digest_row)
...
/-- The extracted `constrain_interactions` map agrees with the hand-written
    SHA bitwise-bus model at `BitwiseBus`. -/
@[Sha2BlockHasherVmAir_Sha256Config_air_simplification]
...
- **Send**: `(+is_digest_row, [composed_hash[0..15], next_gbi])`
  where `composed_hash` is the 8 hash words decomposed into 16 u16 limbs
  (from `hash.a` / `hash.e` bits), and `next_gbi` wraps to 1 on the last block.

- **Receive**: `(-is_digest_row, [prev_hash[0..15], global_block_idx])`
  where `prev_hash` holds 8 words × 2 u16 limbs.

The SHA-256 proof derives and uses the supported-row theorem
...
/-- Composed hash u16 limb for private bus send.
    ...
    Limb `j` ∈ {0, 1} selects low/high 16 bits. -/
@[Sha2BlockHasherVmAir_Sha256Config_constraint_and_interaction_simplification]
...
/-- Private bus send data: 16 composed-hash u16 limbs followed by next_global_block_idx. -/
@[Sha2BlockHasherVmAir_Sha256Config_constraint_and_interaction_simplification]
noncomputable def privateBus_send_data (c : C F ExtF) (row : ℕ) : List F :=
  ((List.range 8).flatMap fun word =>
    (List.range 2).map fun limb =>
      composed_hash_u16 c word limb row)
  ++ [private_bus_next_gbi c row]

/-- Private bus receive data: 16 prev_hash u16 limbs followed by global_block_idx. -/
@[Sha2BlockHasherVmAir_Sha256Config_constraint_and_interaction_simplification]
...
noncomputable def wrapperStateEntry (c : C F ExtF) (row : ℕ)
    : Sha2WrapperBus256StateEntry F where
  multiplicity := -(is_digest_row c row)
  message_type := 0
  request_id := request_id c row
  prev_state_u16 := #v[
    prev_hash c 0 0 row, prev_hash c 0 1 row, prev_hash c 1 0 row, prev_hash c 1 1 row,
    prev_hash c 2 0 row, prev_hash c 2 1 row, prev_hash c 3 0 row, prev_hash c 3 1 row,
    prev_hash c 4 0 row, prev_hash c 4 1 row, prev_hash c 5 0 row, prev_hash c 5 1 row,
    prev_hash c 6 0 row, prev_hash c 6 1 row, prev_hash c 7 0 row, prev_hash c 7 1 row]
  new_state_bytes := #v[
...
noncomputable def wrapperMsg1Entry (c : C F ExtF) (row : ℕ)
    : Sha2WrapperBus256Msg1Entry F where
  multiplicity :=
...
noncomputable def wrapperMsg2Entry (c : C F ExtF) (row : ℕ)
    : Sha2WrapperBus256Msg2Entry F where
  multiplicity :=
    -(wrapperBus_choose2 (encoder_idx c 4 row) * (is_round_row c row + is_digest_row c row))
  message_type := 2
  request_id := request_id c row
  local_msg_bytes := #v[
    compose_schedule_byte c 0 0 row, compose_schedule_byte c 0 1 row,
...
/-- A SHA-side interaction map aligned with the standard OpenVM bus numbering:
    shared bitwise lookup at `BitwiseBus`, wrapper communication at `Sha2WrapperBus`,
    and private chaining at `Sha2PrivateBus`. -/
@[Sha2BlockHasherVmAir_Sha256Config_constraint_and_interaction_simplification]
def constrain_interactions (c : C F ExtF) : Prop :=
  Circuit.buses c = fun index =>
    if index = BitwiseBus then bitwiseBus_trace c
...
lemma constrain_interactions_of_extraction
  ...
  funext index
  rcases eq_or_ne index BitwiseBus with h1 | h1
  ...
  · rcases eq_or_ne index Sha2WrapperBus with h2 | h2
    ...
    · rcases eq_or_ne index Sha2PrivateBus with h3 | h3
      ...
      · unfold Sha2BlockHasherVmAir_Sha256Config.extraction.constrain_interactions at h
        ...
        simp only [BitwiseBus, Sha2WrapperBus, Sha2PrivateBus] at h1 h2 h3
...
lemma bitwiseBus_trace_of_constraints
  ...
  have hbitwise := congrArg (fun buses => buses BitwiseBus) h
  simp [BitwiseBus] at hbitwise
  exact hbitwise
...
lemma privateBus_trace_of_constraints
  ...
  simp [BitwiseBus, Sha2PrivateBus] at hprivate
  exact hprivate
...
lemma wrapperBus_trace_of_constraints
  ...
  simp [BitwiseBus, Sha2WrapperBus] at hwrapper
  exact hwrapper
...
noncomputable def _bitwiseBus_row {C : Type → Type → Type} [Circuit FBB ExtF C]
  ...
  -- Carry entries: 4 slots × 2 limbs
  ((List.range 4).flatMap fun i =>
    (List.range 2).map fun j =>
      bitwiseCarryBusEntry c i j row)
  ++
  -- Digest byte entries: 8 words × 2 pairs
  ((List.range 8).flatMap fun word =>
    (List.range 2).map fun pair =>
...
Convert a global hypothesis on `Circuit.buses c BitwiseBus` into the per-row
`bitwise_lookup_send_properties` predicate, for rows within the trace.
-/
...
noncomputable def bitwise_lookup_send_properties_of_raw_wf
  ...
  intro e he
  -- BusEntry.assume FBB e = (e.multiplicity = 1 → wf_properties e)
  intro hmult
  have ⟨hc_zero, hop_zero⟩ := typed_entry_zero_fields c row e he
  -- The serialised form is in bitwiseBus_row
...
noncomputable def bitwise_lookup_send_properties_of_bus_wf
    ...
    (hwf : ∀ mult a b c_val op,
      (mult, [a, b, c_val, op]) ∈ Circuit.buses c BitwiseBus →
      mult = 1 → a.val < 256 ∧ b.val < 256) :
    bitwise_lookup_send_properties c row := by
