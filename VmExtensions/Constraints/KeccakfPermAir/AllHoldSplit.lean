...
Splits the 3183 row constraints into rot0 (2959 items) and rot1 (224 items)
via 7 contiguous segments.
-/

section allHoldSplit
  variable {C : Type → Type → Type} {F ExtF : Type} [Field F] [Field ExtF] [Circuit F ExtF C]

  /-- rot=0, Control: step init (24 items) -/
  def seg_A (c : C F ExtF) (row : ℕ) : List Prop :=
    [
      constraint_0 c row,
      constraint_1 c row,
      constraint_2 c row,
  ...
  /-- rot=1, Transition: step rotation (24 items) -/
  def seg_B (c : C F ExtF) (row : ℕ) : List Prop :=
    [
      constraint_24 c row,
      constraint_25 c row,
      constraint_26 c row,
  ...
  /-- rot=0, PreimageInit (100 items) -/
  def seg_C (c : C F ExtF) (row : ℕ) : List Prop :=
    [
      constraint_48 c row,
      constraint_49 c row,
      constraint_50 c row,
  ...
  /-- rot=1, Transition: preimage consistency (100 items) -/
  def seg_D (c : C F ExtF) (row : ℕ) : List Prop :=
    [
  ...
  /-- rot=0, Theta + Chi + Iota (2834 items) -/
  def seg_E (c : C F ExtF) (row : ℕ) : List Prop :=
    [
  ...
  /-- rot=1, Transition: output chaining (100 items) -/
  def seg_F (c : C F ExtF) (row : ℕ) : List Prop :=
    [
  ...
  /-- rot=0, Control: export*is_last_step (1 items) -/
  def seg_G (c : C F ExtF) (row : ℕ) : List Prop :=
    [
      constraint_3182 c row
    ]

  set_option maxRecDepth 65536 in
  lemma row_constraint_list_eq_segments (c : C F ExtF) (row : ℕ) :
      row_constraint_list c row =
        seg_A c row ++ seg_B c row ++ seg_C c row ++ seg_D c row ++
        seg_E c row ++ seg_F c row ++ seg_G c row := by rfl

  /-- All rotation-0 constraints (2959 items). -/
  def rot0_constraint_list (c : C F ExtF) (row : ℕ) : List Prop :=
    seg_A c row ++ seg_C c row ++ seg_E c row ++ seg_G c row

  /-- All rotation-1 constraints (224 items). -/
  def rot1_constraint_list (c : C F ExtF) (row : ℕ) : List Prop :=
    seg_B c row ++ seg_D c row ++ seg_F c row

  lemma forall_row_iff_rot0_and_rot1 (c : C F ExtF) (row : ℕ) :
      List.Forall (·) (row_constraint_list c row) ↔
      List.Forall (·) (rot0_constraint_list c row) ∧
      List.Forall (·) (rot1_constraint_list c row) := by
    rw [row_constraint_list_eq_segments]
    simp only [rot0_constraint_list, rot1_constraint_list, List.forall_append]
    tauto

  lemma forall_rot0_of_forall_row (c : C F ExtF) (row : ℕ)
      (h : List.Forall (·) (row_constraint_list c row)) :
      List.Forall (·) (rot0_constraint_list c row) :=
    ((forall_row_iff_rot0_and_rot1 c row).mp h).1

  lemma forall_rot1_of_forall_row (c : C F ExtF) (row : ℕ)
      (h : List.Forall (·) (row_constraint_list c row)) :
      List.Forall (·) (rot1_constraint_list c row) :=
    ((forall_row_iff_rot0_and_rot1 c row).mp h).2
