...
This repository contains an infrastructure for formal verification of OpenVM RISC-V chips in Lean (v4.23.0-rc2), 
together with Lean proofs that the OpenVM implementations of the following RISC-V opcodes satisfy their RISC-V specifications, 
as per the [Official Lean RISC-V specification](https://github.com/opencompl/sail-riscv-lean):
- all 27 opcodes related to the functionalities of the ALU: `ADD`, `ADDI`, `SUB`, `XOR`, `XORI`, `OR`, `ORI`, `AND`, `ANDI`,
  `SLT`, `SLTI`, `SLTU`, `SLTUI`, `SLL`, `SLLI`, `SRL`, `SRLI`, `SRA`, `SRAI`, `MUL`, `MULH`, `MULHU`, `MULHSU`, `DIV`, `DIVU`, `REM`, and `REMU`;
-  7 opcodes related to handling of control flow: `AUIPC`, `BEQ`, `BNE`, `BLT`, `BGE`, `BLTU` and `BGEU`; and
-  2 opcodes related to memory manipulation: `LOADW` and `STOREW`.
...
- the [`OpenvmFv/Fundamentals`](OpenvmFv/Fundamentals) folder contains a number of files that underpin the infrastructure:
  ...
  - [`RV32D.lean`](OpenvmFv/Fundamentals/RV32D.lean), containing lemmas about functions used in the Lean RISC-V specification;
...
- the [report](REPORT-DRAFT.pdf) file contains the **draft report** of the verification effort, including detailed examples and breakdown of assumptions and caveats used.
